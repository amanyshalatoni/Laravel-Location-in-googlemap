...
<form class="row DTForm">
    ...
    <a class="btn btn-success Popup" title="اضافة مستخدم جديد" href="/admin/adminajax/create">
...
<table id="tblAjax" class="table table-striped table-hover">
    <thead>
        <tr><th>اسم المستخدم</th><th>البريد الالكتروني</th><th>رقم الجوال</th><th width="8%">فعال؟</th><th width="16%">تاريخ الانشاء</th>
            <th width="12%"></th>
        </tr>
    </thead>
    <tbody>
    ...
    <script>
        ...
        $(function(){
            ...
            $(document).on("click",".cbActive",function(){
                var id=$(this).val();
                $.get("/admin/adminajax/active/"+id,function(json){
                     ...
                     { data: 'mobile', name: 'mobile' }, 
                       ...
                       { data: 'created_at', name: 'created_at' },                                       
                       {
                           name: 'buttons', "render": function (data, type, row) {
                               return "<a title='تعديل ' href='/admin/adminajax/" + row["id"] + "/edit' class='btn btn-primary Popup btnEdit btn-xs'><i class='glyphicon glyphicon-edit'></i></a> "                              
                               + " <a href='/admin/adminajax/" + row["id"] + "/delete' class='btn Confirm btn-danger btn-xs'><i class='glyphicon glyphicon-trash'></i></a>";
                           }
                       }
               ],
               ajax: {
